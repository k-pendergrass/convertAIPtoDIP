...
del /f /q /s "%%d" 2>> %AIPID%_Deletion-Errors.txt >> %AIPID%_Deleted-Files.txt || echo %%d >> %AIPID%_Deletion-Errors.txt
)
...
del /f /q /s "%%d" 2>> %AIPID%_Deletion-Errors_RR-Only.txt >> %AIPID%_Deleted-Files_RR-Only.txt || echo %%d >> %AIPID%_Deletion-Errors_RR-Only.txt
)
